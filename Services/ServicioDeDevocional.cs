...
namespace ApiPG.Services
{
    public class DevocionalService : IDevocionalService
    {
        ...
        public async Task<DevocionalDto> CreateAsync(CrearDevocionalDto createDto, int createdByUserId)
        {
            var devocional = new Devocional
            {
                ...
                CreadoPorIdUsuario = createdByUserId,
        ...
        public async Task<DevocionalDto?> UpdateAsync(int id, ActualizarDevocionalDto updateDto, int updatedByUserId)
        {
            var devocional = await _context.Devocionales.FindAsync(id);
            if (devocional == null || !devocional.EstaActivo) return null;
            ...
            if (updateDto.IsActive.HasValue) devocional.EstaActivo = updateDto.IsActive.Value;

            devocional.ActualizadoEn = DateTime.UtcNow;
            devocional.ActualizadoPorIdUsuario = updatedByUserId;
        ...
        public async Task<DevocionalDto> SaveStep1Async(DevocionalPaso1Dto step1, int userId)
        {
            try 
            {
        ...
        public async Task<DevocionalDto> SaveStep2Async(int devocionalId, DevocionalPaso2Dto step2, int userId)
        {
            ...
            devocional.ActualizadoEn = DateTime.UtcNow;
            devocional.ActualizadoPorIdUsuario = userId;

            await _context.SaveChangesAsync();
            return await GetByIdAsync(devocionalId) ?? throw new InvalidOperationException("Error al guardar paso 2");
        }

        public async Task<DevocionalDto> SaveStep3Async(int devocionalId, DevocionalPaso3Dto step3, int userId)
        {
            var devocional = await _context.Devocionales.FindAsync(devocionalId);
            if (devocional == null) throw new ArgumentException("Devocional no encontrado");
        ...
        public async Task<DevocionalDto> SaveDraftAsync(GuardarBorradorDto draftDto, int userId)
        {
        ...
        public async Task<IEnumerable<DevocionalDto>> GetByEstadoAsync(DevocionalEstado estado)
        {
            var devocionales = await _context.Devocionales
                .Include(d => d.CreadoPorUsuario)
                .Include(d => d.VoluntarioAsignadoUser)
                .Where(d => d.EstaActivo && d.Estado == estado)
                .OrderByDescending(d => d.CreadoEn)
                .ToListAsync();
        ...
        public async Task<IEnumerable<DevocionalDto>> GetByVoluntarioAsync(int voluntarioId)
        {
            var devocionales = await _context.Devocionales
                .Include(d => d.CreadoPorUsuario)
                .Include(d => d.VoluntarioAsignadoUser)
                .Where(d => d.EstaActivo && d.VoluntarioAsignadoId == voluntarioId)
                .OrderByDescending(d => d.FechaProgramada ?? d.CreadoEn)
                .ToListAsync();
        ...
        public async Task<IEnumerable<DevocionalDto>> GetByFechaAsync(DateTime fecha)
        {
            var devocionales = await _context.Devocionales
                .Include(d => d.CreadoPorUsuario)
                .Include(d => d.VoluntarioAsignadoUser)
                .Where(d => d.EstaActivo && d.FechaProgramada.HasValue && 
        ...
        public async Task<IEnumerable<DevocionalDto>> GetRecentesAsync(int take = 10)
        {
            var devocionales = await _context.Devocionales
                ...
                .Include(d => d.VoluntarioAsignadoUser)
                .Where(d => d.EstaActivo)
                .OrderByDescending(d => d.ActualizadoEn ?? d.CreadoEn)
                .Take(take)
                .ToListAsync();
        ...
        // Utilidades
        public async Task<DevocionalVistaPreviaDto?> GetPreviewAsync(int id)
        {
            var devocional = await _context.Devocionales.FindAsync(id);
            if (devocional == null || !devocional.EstaActivo) return null;
            ...
            return new DevocionalVistaPreviaDto
            {
        ...
        public async Task<IEnumerable<DevocionalDto>> GetByCreatedByUserAsync(int userId)
        {
            var devocionales = await _context.Devocionales
                .Include(d => d.CreadoPorUsuario)
                .Include(d => d.VoluntarioAsignadoUser)
                .Where(d => d.EstaActivo && d.CreadoPorIdUsuario == userId)
                .OrderByDescending(d => d.CreadoEn)
                .ToListAsync();
        ...
        private DevocionalDto MapToDto(Devocional devocional)
        {
            ...
            return new DevocionalDto
            {
                ...
                CreatedAt = devocional.CreadoEn,
                UpdatedAt = devocional.ActualizadoEn,
                CreatedByUserId = devocional.CreadoPorIdUsuario,
                CreatedByUserName = devocional.CreadoPorUsuario?.PrimerNombre + " " + devocional.CreadoPorUsuario?.Apellido,
                IsActive = devocional.EstaActivo
            };
        }
    }
}
